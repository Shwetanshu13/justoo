...
import { justooRiders as riders, orders, orderItems } from "../../db/schema.js";
import {
    eq,
    and,
    count,
    ...
    try {
        ...
        if (!name || !phone || !vehicle_type || !vehicle_number || !password) {
            return errorResponse(
                res,
                "Name, phone, vehicle type, vehicle number, and password are required",
                400
            );
        }
        ...
        if (!validVehicleTypes.includes(vehicle_type)) {
            return errorResponse(
                res,
                "Invalid vehicle type. Must be one of: bike, scooter, car, van",
                400
            );
        }
        ...
        if (!validStatuses.includes(status)) {
            return errorResponse(
                res,
                "Invalid status. Must be one of: active, inactive, busy",
                400
            );
        }
...
export const removeRider = async (req, res) => {
    try {
        ...
        if (!Number.isInteger(numericId) || numericId <= 0) {
            return errorResponse(res, "Invalid rider ID", 400);
        }
...
export const getAllRiders = async (req, res) => {
    try {
        ...
        if (search) {
            conditions.push(
                sql`${riders.username} ILIKE ${"%" + search + "%"} OR ${
                    riders.email
                } ILIKE ${"%" + search + "%"} OR ${riders.name} ILIKE ${
                    "%" + search + "%"
                } OR ${riders.phone} ILIKE ${"%" + search + "%"}`
            );
        }
    ...
    try {
        ...
        if (phone || email) {
            ...
            if (duplicateCheck.length > 0) {
                const field =
                    phone && duplicateCheck[0].phone === phone
                        ? "Phone"
                        : "Email";
        ...
        if (vehicle_number !== undefined)
            updateData.vehicleNumber = vehicle_number;
        if (license_number !== undefined)
            updateData.licenseNumber = license_number;
...
export const getRiderAnalytics = async (req, res) => {
    try {
        ...
        const performanceData = await getRiderPerformance(
            startDate,
            endDate,
            riderId
        );
...
const getRiderStatistics = async () => {
    // Total riders
    const totalRiders = await db.select({ count: count() }).from(riders);
    ...
    return {
        ...
        activeRiders: {
            ...
            percentage:
                total > 0
                    ? ((activeRiders[0].count / total) * 100).toFixed(2)
                    : 0,
        },
        inactiveRiders: {
            ...
            percentage:
                total > 0
                    ? ((inactiveRiders[0].count / total) * 100).toFixed(2)
                    : 0,
        },
...
export const changeRiderPassword = async (req, res) => {
    try {
        ...
        if (!id || !newPassword) {
            return errorResponse(
                res,
                "Rider ID and new password are required",
                400
            );
        }
